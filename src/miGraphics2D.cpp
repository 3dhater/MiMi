...
miGraphics2D::miGraphics2D() {
	...
	m_penSelRect = 0;
	m_rgn_selRect_clip = 0;
}

miGraphics2D::~miGraphics2D() {
	if (m_penSelRect) DeleteObject(m_penSelRect);
	if (m_rgn_selRect_clip) DeleteObject(m_rgn_selRect_clip);
	//if (m_gdi)
	//{
...
void miGraphics2D::Init(yyWindow* window) {
	...
	m_lb_penSelRect.lbStyle = BS_SOLID;
	m_lb_penSelRect.lbColor = RGB(128, 128, 128);
	m_lb_penSelRect.lbHatch = 0;
	m_penSelRect = ExtCreatePen(PS_GEOMETRIC | PS_DASH, 2, &m_lb_penSelRect, 0, NULL);
...
void miGraphics2D::DrawSelectionBox(const v2f& p1, const v2f& p2) {
	...
	MoveToEx(m_window->m_dc, (s32)p1.x, (s32)p1.y, 0);
	LineTo(m_window->m_dc, (s32)p1.x, (s32)p2.y);
	MoveToEx(m_window->m_dc, (s32)p1.x, (s32)p2.y, 0);
	LineTo(m_window->m_dc, (s32)p2.x, (s32)p2.y);
	MoveToEx(m_window->m_dc, (s32)p2.x, (s32)p1.y, 0);
	LineTo(m_window->m_dc, (s32)p2.x, (s32)p2.y);
