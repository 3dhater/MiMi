...
struct v3f
{
	...
	f32* data() { return &x; }
	f32 distance(const v3f& from)const { return v3f(x - from.x, y - from.y, z - from.z).length(); }
	f32 distance(const v4f& from)const { return v3f(x - from.x, y - from.y, z - from.z).length(); }
